...
int main() {
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		case 0:
			...
			RenderText(TextShader, L"SAO CHOI: 3441 ", 25.0f, SCREEN_HEIGHT - 130.0f, 0.35f, glm::vec3(0.7f, 0.7f, 0.11f));

			if (camera.FreeCam)
				RenderText(TextShader, L"CAM TU DO ", SCREEN_WIDTH - 200.0f, SCREEN_HEIGHT - 30.0f, 0.35f, glm::vec3(0.7f, 0.7f, 0.11f));
			if (onFreeCam)
				RenderText(TextShader, L"CAM CO DINH ", SCREEN_WIDTH - 200.0f, SCREEN_HEIGHT - 30.0f, 0.35f, glm::vec3(0.7f, 0.7f, 0.11f));
			break;
		}
		if (PlanetView > 0)
			RenderText(TextShader, L"CAM HANH TINH ", SCREEN_WIDTH - 200.0f, SCREEN_HEIGHT - 30.0f, 0.35f, glm::vec3(0.7f, 0.7f, 0.11f));
		/* PLANET TRACKING + SHOW INFO OF PLANET */
...
void processInput(GLFWwindow *window)
{
	...
	{
		PlanetView = 1;
		Info.Name = L"SAO THUY";
		Info.OrbitSpeed = L"47,87";
		Info.Mass = L"0.32868";
		Info.Gravity = L"0.38";
		onFreeCam = false;
		camera.FreeCam = false;
	...
	{
		PlanetView = 3;
		Info.Name = L"TRAI DAT";
		Info.OrbitSpeed = L"29,76";
		Info.Mass = L"5.97600";
		Info.Gravity = L"1";
		onFreeCam = false;
		camera.FreeCam = false;
	...
	if (glfwGetKey(window, GLFW_KEY_6) == GLFW_PRESS)
	{
		PlanetView = 6;
		Info.Name = L"SAO THO";
		Info.OrbitSpeed = L"9,67";
		Info.Mass = L"561.80376";
		Info.Gravity = L"1.12";
		onFreeCam = false;
		camera.FreeCam = false;
	}
	...
	{
		PlanetView = 7;
		Info.Name = L"SAO THIEN VUONG";
		Info.OrbitSpeed = L"6,84";
		Info.Mass = L"86.05440";
		Info.Gravity = L"0.97";
		onFreeCam = false;
		camera.FreeCam = false;
	}
	...
	{
		PlanetView = 8;
		Info.Name = L"SAO HAI VUONG";
		Info.OrbitSpeed = L"5,48";
		Info.Mass = L"101.59200";
		Info.Gravity = L"1.17";
		onFreeCam = false;
		camera.FreeCam = false;
	}
...
void ShowInfo(Shader &s)
{
	RenderText(s, L"Hanh Tinh: " + Info.Name, 25.0f, SCREEN_HEIGHT - 30.0f, 0.35f, glm::vec3(0.7f, 0.7f, 0.11f));
	RenderText(s, L"Van toc quy dao trung binh (km/s):  " + Info.OrbitSpeed, 25.0f, SCREEN_HEIGHT - 50.0f, 0.35f, glm::vec3(0.7f, 0.7f, 0.11f));
	RenderText(s, L"Khoi luong (kg * 10^24): " + Info.Mass, 25.0f, SCREEN_HEIGHT - 70.0f, 0.35f, glm::vec3(0.7f, 0.7f, 0.11f));
	RenderText(s, L"Trong luc (g): " + Info.Gravity, 25.0f, SCREEN_HEIGHT - 90.0f, 0.35f, glm::vec3(0.7f, 0.7f, 0.11f));
}